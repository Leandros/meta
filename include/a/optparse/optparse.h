...
OPTPARSE_API
void
optparse_print_help(struct optparse_long *longopts)
{
    int i;
    for (i = 0; !optparse_longopts_end(longopts, i); i++) {
        int len = 0;
        struct optparse_long *opt = &longopts[i];
        printf("  ");
        if (opt->longname)
            len += printf("--%s", opt->longname);
        if (opt->longname && opt->shortname)
            len += printf(", ");
        if (opt->shortname)
            len += printf("-%c", opt->shortname);
        if (opt->argtype == OPTPARSE_REQUIRED)
            len += printf(" <arg>");
        else if (opt->argtype == OPTPARSE_OPTIONAL)
            len += printf(" [arg]");
        for (; len <= 24; ++len)
            printf(" ");
        printf("%s\n", opt->description);
    ...
    printf("\n");
    printf("\n");
    printf("Arguments surrounded by < > are required.\n");
    printf("Arguments surrounded by [ ] are optional.\n");
    printf("Arguments followed by ... are repeatable.\n");
}
